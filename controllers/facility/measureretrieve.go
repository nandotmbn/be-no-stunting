...
func FacilityMeasureRetrive() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		cursor, err := recordCollection.Aggregate(ctx, mongo.Pipeline{patientTypeAgg, facilityAgg, dateAgg, idUserAgg, groupStage, checkedAgg})
		if err != nil {
			c.JSON(http.StatusInternalServerError, views.MasterResponse{Status: http.StatusInternalServerError, Message: "error", Data: map[string]interface{}{"data": err.Error()}})
			return
		}
		...
		// display the results
		var resultMonitor []viewsFacility.FacilityMeasureGet
